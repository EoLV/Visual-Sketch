...
def distrchart(data, attrid):
	...
	for i in range(8):
		...
		for j in range(0, f[9] + 1):
			linedata1['Client '+str(i+1)].append(max(f[0][i][j], 0.6))
	...
	for i in range(8):
		...
		for j in range(501):
			antlinedata1['Client '+str(i+1)].append(max(f[7][i][j], 0.6))
	...
	for i in range(8):
		for j in range(8):
			heatmapdata.append({
				...
				'itemStyle': { 'color': getcolor(max((f[0][i][j] / matrixmax - 0.5) * 2, 0),
...
def sidebarchart(data, attrid):
	...
	n = len(attr)
	sidebardata = []
	for i in sorted(attr.keys(), reverse=True):
		now = {
			'name': attr[i][0],
			'value': [i, attr[i][2]],
			'time': attr[i][1],
	...
	return { 'sidebardata': sidebardata, 'text': [ { 'name': 'TIME (Data _)', 'value': 1 } ], 'minvalue': 0 }
